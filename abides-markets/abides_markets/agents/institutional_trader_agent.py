...
class InstitutionalTraderAgent(TradingAgent):
    ...
    def wakeup(self, current_time: NanosecondTime) -> None:
        ...
        # if self.mkt_closed and self.symbol not in self.daily_cAlose_price:
    ...
    def placeOrder(self, current_time: NanosecondTime) -> None:
        ...
        if current_time >= self.trigger_time and self.symbol in self.holdings and self.holdings[self.symbol] > 0:
            ...
            #     self.sell_amount = total_transacted_volume
            #     self.sell_amount = 10

            # order_size = min(self.holdings[self.symbol], int(self.sell_amount * self.sell_volume_factor * self.inventory))
